...
async function getMenus(req,res){
    ...
    if(active === undefined){  // Si esta indefinido el menu (el usuario no ingreso un usuario en especifico) entonces devuelve todos
        response = await Menu.find().sort({order: "asc"});
    }else{
        response = await Menu.find({active}).sort({order: "asc"}); // Devuelve los activos 
    } 
